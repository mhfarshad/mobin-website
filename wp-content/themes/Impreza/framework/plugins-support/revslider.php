...
function us_include_revslider_js_for_row_bg() {
	...
	if ( $isPutIn == FALSE AND $includesGlobally == "off" ) {
		$post_content = '';
		$titlebar_footer_content = us_get_current_titlebar_footer_content();
		if ( is_singular() ) {
			$post = get_post( get_the_ID() );
			$post_content = $post->post_content;
		}

		if ( ( $post_content != '' AND stripos( $post_content, 'us_bg_slider=' ) !== FALSE ) OR ( $titlebar_footer_content != '' AND ( stripos( $titlebar_footer_content, 'us_bg_slider=' ) !== FALSE OR stripos( $titlebar_footer_content, '[rev_slider' ) !== FALSE ) ) ) {
