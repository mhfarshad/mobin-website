...
$output .= '<a class="w-search-open" href="javascript:void(0);" title="' . us_translate( 'Search' ) . '">';
if ( ! empty( $icon ) ) {
...
$output .= '</div>';
if ( $layout == 'simple' ) {
	$output .= '<button class="w-search-form-btn" type="submit" title="' . us_translate( 'Search' ) . '">';
	if ( ! empty( $icon ) ) {
		$output .= us_prepare_icon_tag( $icon );
	}
	$output .= '</button>';
...
$output .= '<a class="w-search-close" href="javascript:void(0);" title="' . us_translate( 'Close' ) . '"></a>';
$output .= '</form></div></div>';
