...
$us_gb_product_field_types = array(
	'price' => us_translate( 'Product price.', 'woocommerce' ),
	'rating' => us_translate( 'Product ratings', 'woocommerce' ),
	'sku' => us_translate( 'Product SKU.', 'woocommerce' ),
	'weight' => us_translate( 'Weight', 'woocommerce' ),
	'dimensions' => us_translate( 'Product dimensions.', 'woocommerce' ),
	...
	function usgb_get_elms_placeholders( &$layout, &$data, $place ) {
		...
		foreach ( $layout[ $place ] as $elm ) {
			...
			if ( substr( $elm, 1, 7 ) == 'wrapper' ) {
				...
				$type = strtok( $elm, ':' );
				$elm_title = us_config( 'grid-settings.elements.' . $type . '.title', $type );
...
foreach ( us_config( 'grid-settings.elements', array() ) as $type => $elm ) {
...
foreach ( us_config( 'grid-settings.elements', array() ) as $elm => $elm_config ) {
