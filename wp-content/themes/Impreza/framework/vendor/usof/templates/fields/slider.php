...
$output .= '>';
$valueDecimalPart =  strval( fmod( abs( floatval( $value ) ), 1 ) );
if ( ! empty( $valueDecimalPart[3] ) ) {
	$valueOutput = number_format( floatval( $value ), 2 );
} else if ( ! empty( $valueDecimalPart[2] ) ) {
	$valueOutput = number_format( floatval( $value ), 1 );
} else {
	$valueOutput = number_format( floatval( $value ), 0 );
}
...
$range_offset_style = ( is_rtl() ) ? 'right' : 'left';
$output .= '<div class="usof-slider-range" style="' . $range_offset_style . ':' . $offset . '%;"><div class="usof-slider-runner" draggable="true"></div></div>';
$output .= '</div></div>';
$output .= '</div>';
